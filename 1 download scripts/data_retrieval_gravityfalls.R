...
gravityfalls_overview_urls<-c('https://gravityfalls.fandom.com/wiki/Transcripts')
gravityfalls_prefix <- 'https://gravityfalls.fandom.com/'
gravityfalls_transcript_urls <- get_transcripts_urls(gravityfalls_overview_urls, gravityfalls_prefix)
for(i in 1:length(gravityfalls_transcript_urls)){
  Sys.sleep(1)
  get_transcript(gravityfalls_transcript_urls[i], paste('.\\data\\gravityfalls\\',str_match(gravityfalls_transcript_urls[i], "wiki/\\s*(.*?)\\s*/Transcript")[2], sep = ""))
}
