...
peppapig_overview_urls<-c('https://peppapig.fandom.com/wiki/Category:Transcripts')
peppapig_prefix <- 'https://peppapig.fandom.com/'
peppapig_transcript_urls <- get_transcripts_urls(peppapig_overview_urls, peppapig_prefix)
for(i in 1:length(peppapig_transcript_urls)){
  Sys.sleep(1)
  get_transcript(peppapig_transcript_urls[i], paste('.\\data\\peppapig\\',str_match(peppapig_transcript_urls[i], "wiki/\\s*(.*?)\\s*/Transcript")[2], sep = ""))
}
