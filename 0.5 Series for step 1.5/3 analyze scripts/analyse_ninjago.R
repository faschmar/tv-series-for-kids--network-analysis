...
process_transcript<-function(filepath){
  ...
  sentimentNinjago <- sentiment_score(script[, 2])
  sentimentProCharacter <- aggregate(sentimentNinjago, by = list(script[, 1]), FUN = mean)
  sentimentProCharacter <- aggregate(sentimentNinjago, by = list(script[, 1]), FUN = length)
  ...
  ep_sentiment_plot<-assign(paste(series, title_intermediate,"sentiment", sep="_"),sentimentNinjago)
  ...
  #ts.plot(sentimentNinjago, main = file)s
  
  #write to pdf
  pdf(file=paste("./data/ninjago/plots/", series, "_", title_intermediate, ".pdf", sep=""))
  # 2. Create the plot
  plot(ep_sociogram_plot, main = "plot: sociogram", sub = paste(series, title_intermediate,"sociogram", sep="_"))
  ts.plot(ep_sentiment_plot, main = "plot: sentiment score per lines (time)", sub = paste(series, title_intermediate,"sentiment", sep="_"), xlab="line", ylab="sentiment score")
  # 3. Close the file
...
# Coming soon
files <- list.files(path='./data/ninjago/processed', full.names=TRUE, recursive=TRUE, include.dirs=FALSE)
for (file in files){
  process_transcript(file)
}

#cleanup
#rm(characterName, ep_sociogram_plot, filetext, pairs, script, sentimentProCharacter, sociogram_intermediate, characters, ep_sentiment_plot, file, file_intermediate, filepath, files, myEdges, nNodes, sentimentNinjago, title_intermediate, transcript_lines) 
