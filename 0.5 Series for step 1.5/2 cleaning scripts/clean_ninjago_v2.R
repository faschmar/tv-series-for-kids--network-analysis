...
clean_transcript <- function(filepath){
  ...
  for (line in transcript_lines){
    ...
    line <- gsub("|Title=Codename: Acturus", "", line)
    line <- gsub("|Next=Codename: Arcturus", "", line)
    line <- gsub("|Previous=Operation Heavy Metal: Machia", "", line)
  ...
  if (length(cleaned_transcript) < 20){
    file.move(filepath, "./data/ninjago/abandoned/")
    return ()
  }
  file.move(filepath, "./data/ninjago/raw/")
  filepath <- gsub("ninjago", "ninjago/processed", filepath)
  writeLines(cleaned_transcript, filepath)
}

files <- list.files(path='./data/ninjago/', full.names=TRUE, recursive=TRUE, include.dirs=FALSE)
for (file in files){
