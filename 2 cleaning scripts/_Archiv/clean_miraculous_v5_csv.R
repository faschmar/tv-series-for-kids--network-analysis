...
clean_transcript <- function(filepath){
  #filepath<-"./data/miraculous/Stormy Weather.txt"
  ...
  script <- matrix(unlist(characterName), ncol = 2, nrow=length(characterName), byrow = T)
  script_cleaned <-script
  #script_cleaned <-script[order( script[,1] ), ]
  script_cleaned<-data.frame(script_cleaned)
  script_cleaned$X1 <- trimws(script_cleaned$X1, which = c("both"))
  #script_cleaned <-script_cleaned[order(script_cleaned$X1),]
  #newfilepath<-sub("./data/miraculous/processed/","./data/miraculous/cleaned/", filepath)
  newfilepath<-sub("txt","csv", filepath)
  write.table(script_cleaned,newfilepath, row.names = F, append = T, col.names = F, sep = "|")
  if (nrow(script_cleaned) < 20){
    write.table(script_cleaned,newfilepath, row.names = F, append = T, col.names = F, sep = "|")
    return ()
  }
}
...
#files <- list.files(path='./data/miraculous', full.names=TRUE, recursive=F, include.dirs=FALSE)
