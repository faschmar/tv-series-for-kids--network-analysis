...
colnames(ep_df_values_wholeseason) <- data.frame("title", "season", "ep_per_season", "ep_overall", "air_date", "ep_edge_density_value", "ep_reciprocity_value", "ep_diameter_value")
  ...
  #filepath<-"./data/miraculous/processed/Guiltrip.txt" # ändern
  #filepath<-"./data/miraculous/processed/Captain Hardrock.txt" # ändern
  #filepath<-"./data/miraculous/processed/Cat Blanc.txt" # ändern
  filepath<-"./data/miraculous/processed/Ephemeral.txt" # ändern
  ...
  ep_pairs <- matrix(NA, ncol = 2, nrow = length(ep_characters)-1)
  ep_pairs[, 1] <- ep_characters[1:(length(ep_characters)-1)]
  ep_pairs[, 2] <- ep_characters[2:length(ep_characters)]
